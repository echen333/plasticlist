...
class TSVProcessor:
    def __init__(self, index_name: str = "plasticlist3"):
        ...
        self.important_columns = [
            'id', 'product_id', 'product', 'tags', 
            'triplicate_1_sample_id', 'triplicate_2_sample_id',
            'lot_no', 'manufacturing_date', 'expiration_date', 
            'collected_on', 'collected_at', 'collection_notes',
            'blinded_name', 'blinded_photo', 'shipped_on',
            'shipped_in', 'shipment_type', 'arrived_at_lab_on',
            'analysis_method_phthalates', 'analysis_method_bisphenols'
        ]
        ...
        for col in self.important_columns:
            if pd.notna(row[col]) and row[col] != '':
                ...
                if value:
                    ...
                    logger.debug(f"Added field {col}: {value[:100]}...")  # Show first 100 chars
    ...
    def save_vectors(self, vectors: List[Dict], filepath: str = "utils/tsv_embeddings.txt"):
        ...
        if vectors:
            try:
                ...
                for i in range(0, len(vectors), batch_size):
                    ...
                    logger.info(f"Upserting batch {i//batch_size + 1}/{(len(vectors)-1)//batch_size + 1}")
                    self.index.upsert(
                        vectors=batch,
                        namespace="default"
                    )
